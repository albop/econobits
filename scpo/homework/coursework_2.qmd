---
title: Coursework 2026 - Consumption Saving in General Equilibrium
format:
    html: default
    ipynb: default
---
...
#### Optional: Endogenous Grid Points Method

As an alternative to value-function iteration, one can solve the household problem using the Euler equation together with the endogenous grid points method (EGM).

Define end-of-period savings by $k=a-c$. Then next period's available income is:

$$a' = {\color{red}r} k + {\color{red}w} e'$$

and the Euler equation for an interior solution becomes:

$$U'(c(a)) = \beta {\color{red}r} \, \mathbb{E}\left[ U'(c(a')) \right]$$

With CRRA utility, this can be rewritten as:

$$c(a) = \left( \beta {\color{red}r} \, \mathbb{E}\left[c(a')^{-\gamma}\right] \right)^{-1/\gamma}$$

The idea of EGM is to start from a grid for end-of-period savings $k$, compute the implied consumption from the Euler equation, and then recover the corresponding current asset level $a=c+k$. This produces an *endogenous* grid for current assets.

::: {.callout-note}
The borrowing constraint is $k\geq 0$. When it binds, the household consumes all available resources, so that $c(a)=a$.
:::

@. Write the Euler equation associated with the household problem and explain why, with CRRA utility, it can be inverted to recover current consumption from expected marginal utility tomorrow.

```markdown
your text here
```

@. Create a grid `kgrid` for end-of-period savings. For each `k` in `kgrid`, define a function that computes the corresponding endogenous current asset level

$$a = k + \left( \beta {\color{red}r} \sum_{\ell} \omega_{\ell} \, c(a'_{\ell})^{-\gamma} \right)^{-1/\gamma}$$

where $a'_{\ell} = {\color{red}r} k + {\color{red}w} e_{\ell}$ and $(e_{\ell},\omega_{\ell})$ are the quadrature nodes and weights.

```julia
# your code here
```

@. Write a function `egm_step(cvec, m, p, dis)::Tuple{Vector, Vector}` which takes a consumption policy tomorrow, performs one EGM update, and returns:

- the endogenous asset grid
- the updated consumption values on that grid

Then interpolate this updated policy back onto the original asset grid `dis.g`.

```julia
# your code here
```

@. Explain how you handle the borrowing constraint for asset levels below the smallest endogenous grid point. Implement this treatment in your code.

```julia
# your code here
```

@. Write a function `egm(m, p, dis)` which iterates on the consumption rule until convergence. Compare the resulting policy function and execution time with value-function iteration and policy-function iteration.
...
::: {.callout-note title="Trembling hand"}
