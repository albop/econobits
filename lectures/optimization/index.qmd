...
<img src="nonsmooth.png" width=30%>

It certainly doesn't meet any strong regularity condition. And yet, neural networks do great things!


## Plan

We'll review many optimization algorithms:

- [one-dimensional root-finding](#one-dimensional-root-finding)
- [one-dimensional optimization](#one-dimensional-minimization)
- [local root-finding](#unconstrained-multidimensional-root-finding)
- [local optimization](#unconstrained-multidimensional-minimization)
- [constrained optimization](#constrained-optimization-and-complementarity-conditions)
- [constrained root-finding](#solution-strategies-for-ncp-problems)


## Optimization method map

| Problem | First tool to try | Why | Watch out |
|---|---|---|---|
| 1D root, bracket known | (Grid Search) Bisection | ✅ guaranteed convergence | ⚡ can be slow |
| 1D root, smooth + good guess | Newton | ⚡ very fast locally | ⚠️ sensitive to initial guess |
| 1D minimum on interval | (Grid Search) Golden section | ✅ derivative-free + robust | ⚡ slower than Newton-like |
| Multidim root | Multidim Safeguarded Newton | ⚡ very fast locally | ⚠️ sensitive to initial guess |
| Multidim smooth minimization | Gradient / Quasi-Newton | 🛠️ scalable and practical | ⚠️ scaling and step-size matter |
| Constraints/complementarity | KKT + NCP solver | ✅ handles corners naturally | ⚠️ formulation can be tricky |

(Grid Search is not really a method, but it is robust and very easy to implement).
...
Assume you look for a root $x^{\star}$ of $f$ in $[a,b]$.

__Grid Search Algorithm__

- Choose a grid of $N$ evenly spaced points: $x_i = a+i\frac{b-a}{N}$ for $i=0,...,N$.
- Evaluate $f$ at each point and check if there is a sign change between two consecutive points.
- If there is a sign change, then there is a root in the interval $[x_i, x_{i+1}]$.
  - best guess: $\overline{x} \approx \frac{x_i+x_{i+1}}{2}$ 
- If there are multiple sign changes, you can find multiple roots.

__Convergence__:

- by construction $|\overline{x}-x^{\star}| \leq \frac{b-a}{N}$
- no guarantee on $f(x^{\star})$:
  - if too large, increase $N$ and try again
  - or reapply the algorithm on $[x_i, x_{i+1}]$

## Bisection

Find $x \in [a,b]$ such that $f(x) = 0$. Assume $f(a)f(b) <0$.

__Bisection Algorithm__
...
<img src="bisection_progress.png" width=82%>

- Red bars: shrinking brackets $[a_n,b_n]$
- Green dots: midpoints tested each iteration
...
⚠️ Newton is powerful, but fragile far from the root.
...
## Newton: bad initial guess

<img src="newton_success_failure.png" width=90%>

- Left: good initial guess, fast convergence ✅
- Right: poor guess, unstable/cycling behavior ⚠️


## Newton Overshoot Intuition {#newton-overshoot-intuition}

![](newton_overshoot.png){width=82%}

- First step goes in the correct direction (toward the root)
- But the tangent approximation is too aggressive and jumps past it
...
🛠️ Think "small steps downhill"; robustness depends on step size.
...
You need to solve a smooth 1D equation and you have a reliable bracket $[a,b]$.

- Option A: Newton from the midpoint
- Option B: Bisection on $[a,b]$

Best first move for robustness: **B** ✅

Then switch to Newton close to the solution for speed ⚡.


## Convergence cheat sheet

| Method | Info needed | Typical local rate | Robustness |
|---|---|---|---|
| Bisection | signs at endpoints | linear | very high |
| Secant | function values | superlinear | medium |
| Newton | first derivative / Jacobian | quadratic | medium-low |
| Gradient descent | gradient | linear | medium-high |
| Newton minimization | gradient + Hessian | quadratic | medium-low |
...
🧠 KKT tells you whether optimum is interior or at a corner (binding constraint).
...
<img src="kkt_geometry.png" width=86%>

- Left panel: interior optimum from tangency
- Right panel: inequality constraints can force a corner solution
