...
|---|---|---|---|
| 1D root, bracket known | Bisection | ✅ guaranteed convergence | ⚡ can be slow |
| 1D root, smooth + good guess | Newton/Secant | ⚡ very fast locally | ⚠️ sensitive to initial guess |
| 1D minimum on interval | Golden section | ✅ derivative-free + robust | ⚡ slower than Newton-like |
...
## How to read each method slide

- 🧠 Intuition: what geometry the method exploits
- ✅ Guarantee: when convergence is ensured
- ⚡ Speed: local convergence rate (linear/superlinear/quadratic)
- ⚠️ Failure mode: typical reason it breaks
- 🛠️ Fix: one practical stabilization trick
...
🧠 Split the interval in two and keep the side where the sign changes.
...
⚡ Local tangent approximation gives very fast convergence near the solution.
...
## Newton: fast when local, fragile when not
